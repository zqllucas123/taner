...
- [x] `pricing`（AI 定价助手）：黄金定价文案 + AI 客源偏好 violet 面板 + 品类 emoji（`7e52a10`）
- [x] `product-manage`（商品管理）：「当前摆摊货物」标题 + 已售数展示（`bb43e9e`）
- [x] `order-manage`（订单管理）：新增简易经营台账·大白话营收表（设计稿 ledger 视图）（`1ae3d75`）
- [x] `stall-setting` / `coupon-manage` / `stall-qrcode`：配色已统一，结构与设计稿无重大差异
- **验收**：✅ 摊主端结构/文案对齐；amber 辅色用于摊主端强调元素。

### 阶段 4 · 主包 + 我的页 ✅ 已完成
- [x] `pages/index`（启动分发页）配色迁移
- [x] `pages/my`（我的/双角色入口）：等级徽章 + 三宫格指标 + 带副文案角色切换（`f5cbb01`）
- **验收**：✅ 双角色入口、大字模式样式对齐。

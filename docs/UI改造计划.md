...
- **验收**：✅ 随 `d4b9451` 全站 token 迁移完成。

### 阶段 2 · 顾客端逐页改造 ✅ 已完成（含结构/文案级对齐）
- [x] `home`（地图扫街）：结构级对齐设计稿 map 视图 + mock 摊位兜底（`211e5c7`）
- [x] `stall-detail`（摊位详情）：结构级对齐设计稿 shop 视图（`d6da9d5`）
- [x] `reserve`（预定下单）：自提凭证文案 + 极简预定规则 amber 提示框（`e279e14`）
- [x] `orders`（我的预定）：待自提角标 + 条形码凭证 + 状态语义色（`795d1df`）
- [x] `my-coupons`（我的优惠券）：rose 券视觉（撕口券卡 + rose token）（`afc43b4`）
- [x] `wish-pool`（许愿池）：双向许愿池文案 + emerald 回复气泡 + 备货占位框（`67676d1`）
- **验收**：✅ 顾客端 6 页配色统一 emerald 体系，结构/文案对齐设计稿。

### 阶段 3 · 摊主端逐页改造 🔶 配色已完成，结构级对齐进行中
- [x] 六页配色 token 迁移（随 `d4b9451` 完成）
- [ ] `pricing`（AI 定价助手）：结构对齐设计稿 assistant 视图（AI 结果卡、推理文案区，设计稿亮点）
- [ ] `product-manage`（商品管理）：结构对齐设计稿 products 视图
- [ ] `order-manage`（订单管理）：结构/文案对齐（含 ledger 流水概念）
- [ ] `stall-setting` / `coupon-manage` / `stall-qrcode`：结构/文案细化
- **验收**：摊主端 6 页结构/文案对齐；amber 辅色用于摊主端强调元素。

### 阶段 4 · 主包 + 我的页 🔶 配色已完成，结构级对齐待做
- [x] `pages/index`（启动分发页）配色迁移
- [ ] `pages/my`（我的/双角色入口）：对齐设计稿 me 视图（渐变头像卡、三宫格指标、提货凭证卡、角色切换开关）
- **验收**：双角色入口、大字模式样式对齐。
